...
% 
% Vegard Antun, 2019.
function [idx, str_id] = cil_spf2_2level(N, nbr_samples, p_norm, r_factor)
    if nargin < 3
        p_norm = inf;
    end
    str_id = '2level';    
    ...
    % We do not create a first level with exactly `half_nbr_samples`,
    % but it will be approximatly equal.
    if (p_norm == inf)
        r = round(sqrt(half_nbr_samples));
        Z = zeros([N,N], 'uint8');
        sx = round((N-r)/2);
        Z(sx:sx+r-1, sx:sx+r-1) = uint8(1);
        str_id = '2level_l_inf';    
    elseif p_norm == 2
        r_init = sqrt(half_nbr_samples/pi);
        shapef = cil_shape_ball([round(N/2), round(N/2)], r_init, p_norm); 
        Z = uint8(cil_shape(shapef, N, 1));
        str_id = '2level_l_2';    
    elseif p_norm == 1
        r_init = sqrt(half_nbr_samples/2);
        shapef = cil_shape_ball([round(N/2), round(N/2)], r_init, p_norm); 
        Z = uint8(cil_shape(shapef, N, 1));
        str_id = '2level_l_1';    
    else
