...
%          required by idx(k). 
%
function [idx, scales] = sph1_rect3(N, M, nbr_samples, J0)
    ...
    bin_sizes_reversed = [bin_sizes_reversed, 2^(J0+1)];

    a = floor((nbr_samples-2^(J0+1))/(r));
    bin_samples_reversed = ones(1,r)*a;
    bin_samples_reversed(1) = a;
    bin_samples_reversed(end) = 2^(J0+1);
