% cil_sample_gauss_wavelet() performs a compressive sensing experiment with a 
% Gaussian sampling operator and a Daubechies wavelet as the sparsifying operator
% The entries in the sampling matrix A are drawn from a Gaussian distribution 
...
%
% The function stores the reconstructed image as 'filename' with the file 
% extension specified by cil_dflt.image_format (see etc/set_defaults.m)
%
% INPUT
% im       - Image which will be sampled
% subsamplin_rate - Fraction between the number of samples  and number of
...
% 
function [im_rec, z] = cil_sample_gauss_wavelet(im, subsampling_rate, sigma, filename, vm, varargin);
