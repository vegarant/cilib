...
%     minimize_{z ∈ ℝ^{N}} lambda*||z||_{1,w} + ||measurements - A*z||_{2}^{2}
...
%       transform.
% weights    - Weights for the weighted l1-norm.
...
%
function sol = cil_FISTA_solver(measurements, opA, weights, nbr_of_itr, L, lambda)
    ...
    for i = 1:nbr_of_itr
        ...
        x = sign(z).*max(weights.*abs(z) - (lambda/L), 0)./weights;
        t = (1 + sqrt(1+4*tp^2))/2;
        y = x + ((tp - 1)/t)*(x - xp);
        tp = t;
        yp = y;
