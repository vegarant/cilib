% This configuration file set all parameters used by `Compute_PSNR_curve_walsh.m`
...
r0 = 2;
nbr_levels = 50;
f1 = @(N, nbr_samples) cil_sph2_gcircle(N, nbr_samples, a, r0, nbr_levels);
...
r0 = 2;
nbr_levels = 50;
f2 = @(N, nbr_samples) cil_sph2_gcircle(N, nbr_samples, a, r0, nbr_levels);
...
radius = 2;
p_norm = 0.5;
f3 = @(N, nbr_samples) cil_sph2_exp(N, nbr_samples, a, r0, nbr_levels, radius, p_norm);
...
radius = 2;
p_norm = 0.5;
f4 = @(N, nbr_samples) cil_sph2_exp(N, nbr_samples, a, r0, nbr_levels, radius, p_norm);
...
f5 = @(N, nbr_samples) cil_sph2_DAS(N, nbr_samples, sparsity);
str_id5 = sprintf('DAS, epsilon: %g', epsilon);
color_id5 = {cil_dflt.yellow, '-'};


f6 = @(N, nbr_samples) cil_sph2_DIS(N, nbr_samples, sparsity);
